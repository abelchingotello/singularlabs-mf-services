...
export class AssingIndividualComponent implements OnInit {
  ...
  registerServiceAssign() {
    ...
    let dataRegister = this.data.map(value => ({
      ...
      comissionFixed: this.dataServiceSelected.fixedcomission - this.fixed.value, //number
  ...
  getRecaudador() {
    this.spinner.spinnerOnOff();
    this.personService.getPerson('RECAUDADORA DE SERVICIOS', undefined).subscribe({
      next: (response) => {
